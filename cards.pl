...
energy_card(energy(name(Name),type(EnergyType))) --> [string(Name), colon, atom(energy),colon], energy_cat(EnergyType).
...
trainer_card(trainer(name(Name),type(TrainerType),ability(Id))) --> [string(Name), colon, atom(trainer), colon, atom(cat),colon], trainer_type(TrainerType), [colon, int(Id)].
trainer_type(item) --> [atom(item)].
trainer_type(supporter) --> [atom(supporter)].
trainer_type(stadium) --> [atom(stadium)].

pokemon_card(pokemon(name(Name), stage(PokemonType), evolves(EvFrom), energy(EnergyType), hp(InitialHP), RetreatCost, Attacks)) -->
...
retreat_cost(retreat(type(nil),amount(0))) --> [].
retreat_cost(retreat(type(EnergyType),amount(NCards))) --> [colon,atom(retreat),colon], energy_cat(EnergyType), [colon, int(NCards)].

attacks(attacks(L)) --> [atom(attacks),colon], attack_list(L).
...
requirement(req(energy(EnergyType),amount(NCards))) --> energy_cat(EnergyType), [colon,int(NCards)].
